    ...
    public MyPanel (Game g) {
      ...
      this.addMouseListener (new MouseAdapter () {
        public void mouseClicked (MouseEvent me) {
          Point p = me.getPoint ();
          if (game.movePlayer (new Position (p.x / ICON_W, p.y / ICON_H))) {
            repaint ();
      ...
      for (int y = 0; y < game.board.ySize; y++) {
        for (int x = 0; x < game.board.xSize; x++) {
          ...
          if (game.player.position.equals (new Position (x, y))) {
            ...
            if (game.board.items[x][y] instanceof Wall)
              image = wallImage;
            if (game.board.items[x][y] instanceof Crate)
              image = game.board.items[x][y].isMarked ()? crateMarkedImage : crateImage;
            if (game.board.items[x][y] instanceof Ground)
              image = game.board.items[x][y].isMarked ()? blankMarkedImage : blankImage;
          }
